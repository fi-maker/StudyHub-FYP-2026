...
class ApplicationHistory02 extends StatelessWidget {
  const ApplicationHistory02({Key? key}) : super(key: key);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Row(children: [
          ...
          const Text("Scholarship Apps", style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
        ]),
        backgroundColor: primaryTeal,
        elevation: 0,
      ),
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('scholarships_applications').where('uid', isEqualTo: uid).snapshots(),
          ...
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: pureWhite, shape: BoxShape.circle),
                  child: Icon(Icons.school_outlined, size: 50, color: textSecondary)),
              const SizedBox(height: 16),
              const Text("No scholarship apps", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
              const SizedBox(height: 4),
              Text("Your applications will appear here", style: TextStyle(fontSize: 12, color: textSecondary)),
            ]));
          }
                  ...
                  child: Column(children: [
                    InkWell(
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) =>
                          ApplicationDetail(applicationData: data, applicationNumber: index + 1))),
                      child: Row(children: [
                    ...
                    if (status == 'accepted' || status == 'rejected') ...[
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          status == 'accepted' ? const AgreementPage() : const CompleteStudentRegistrationFormScholarship())),
